            ...
            <div key={l.id} className="glass-card p-4">
              <div className="flex items-start gap-3 mb-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground truncate">{l.lender || "—"} · due day {l.due_day}</div>
                </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="text-2xl font-bold num">{fmtMoney(l.monthly_amount, currency)}</div>
                <div className="flex items-center gap-2">
                  {overdue && !isPaid && (
                    <span className="flex items-center gap-1 text-destructive text-xs"><AlertCircle size={14} /> Overdue</span>
                  )}
                  {l.remaining_payments && <span className="text-muted-foreground text-xs">{l.remaining_payments} left</span>}
                </div>
              </div>
            </div>
          );
