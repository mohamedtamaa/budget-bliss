      ...
      <div className="glass-card divide-y divide-border/40">
        {items.length ? items.map((i: any) => (
          <div key={i.id} className="flex items-center gap-3 p-3 sm:p-4">
            {i.active && (
              <PaidToggle sourceType="subscription" sourceId={i.id} amount={Number(i.amount)} month={month} />
            )}
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground truncate">since {i.start_date}{i.end_date ? ` · until ${i.end_date}` : ""}</div>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <div className="font-semibold num text-sm">{fmtMoney(i.amount, currency)}</div>
