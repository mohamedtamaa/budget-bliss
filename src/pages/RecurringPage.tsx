...
export default function RecurringPage() {
  ...
  const reset = () => { setForm({ name: "", type: "expense", amount: "", start_date: format(new Date(), "yyyy-MM-dd"), end_date: "", account_id: "", category_id: "", active: true, included_in_total: true }); setEditing(null); };

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload: any = {
      name: form.name, type: form.type, amount: Number(form.amount),
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) reset(); }}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>{editing ? "Edit" : "New"} recurring item</DialogTitle></DialogHeader>
      ...
      <div className="glass-card divide-y divide-border/40">
        ...
        )) : <div className="p-10 text-center text-sm text-muted-foreground">No recurring items yet</div>}
      </div>
    </div>
  );
}
